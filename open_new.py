...
from postgres import RandomNumberDB
import uuid
import subprocess
...
class MultiStepApp:
    def __init__(self, root):
        ...
        self.db = RandomNumberDB(dbname='postgres', user='postgres', password='pigeon123!')
        self.db.connect()
        ...
        self.punishment_period = tk.StringVar(value="0")
        ...
        self.rounds_before_warning = tk.StringVar(value="1")
    ...
    def increment_value(self, variable, increment):
        try:
            ...
            if variable == self.reinforcement_ratio and new_value >= 1:
                ...
                variable.set(new_value)
            elif variable == self.punishment_period and new_value <= 100:
                variable.set(new_value)
            elif variable == self.feed_time and 1 <= new_value <= 10:
                variable.set(new_value)
            elif variable == self.consecutive_warnings_limit and 1 <= new_value <= 10:
                variable.set(new_value)
            elif variable == self.rounds_before_warning and 1 <= new_value <= 10:
                variable.set(new_value)
    ...
    def decrement_value(self, variable, decrement):
        try:
            ...
            if variable == self.reinforcement_ratio:
                ...
                if current_value > 0:
                    variable.set(max(0, current_value - decrement))
            elif variable == self.punishment_period:
                if current_value > 0:
                    variable.set(max(0, current_value - decrement))
                ...
                if current_value > 1:
                    variable.set(max(1, current_value - decrement))
            elif variable == self.rounds_before_warning:
                if current_value > 1:
                    variable.set(max(1, current_value - decrement))
    ...
    def show_more_options(self):
        ...
        # Punishment Period
        if not self.is_basic_training_mode() and not self.is_schedule_training_mode():
            tk.Label(input_frame, text="Punishment Period (seconds):", font=("Tahoma", 12)).grid(row=1, column=0, pady=2, sticky="e")
            self.punishment_period_entry = tk.Entry(input_frame, textvariable=self.punishment_period, validate="key", validatecommand=(self.root.register(self.validate_between_0_and_100), '%P'), width=5)
            self.punishment_period_entry.grid(row=1, column=1, pady=2, sticky="w")
            tk.Button(input_frame, text="+", command=lambda: self.increment_value(self.punishment_period, 1)).grid(row=1, column=2, padx=2)
            tk.Button(input_frame, text="-", command=lambda: self.decrement_value(self.punishment_period, 1)).grid(row=1, column=3, padx=2)
            tk.Button(input_frame, text="+10", command=lambda: self.increment_value(self.punishment_period, 10)).grid(row=1, column=4, padx=2)
            tk.Button(input_frame, text="-10", command=lambda: self.decrement_value(self.punishment_period, 10)).grid(row=1, column=5, padx=2)
            self.punishment_period_entry.bind("<FocusOut>", lambda e: self.validate_entry(self.punishment_period, 0, 100))
        ...
        # Regular Rounds Before Warning Signal Training
        if not self.is_basic_training_mode() and not self.is_schedule_training_mode() and not self.is_random_warning_mode():
            tk.Label(input_frame, text="Rounds Before Warning:", font=("Tahoma", 12)).grid(row=5, column=0, pady=2, sticky="e")
            self.rounds_before_warning_entry = tk.Entry(input_frame, textvariable=self.rounds_before_warning, validate="key", validatecommand=(self.root.register(self.validate_between_1_and_10), '%P'), width=5)
            self.rounds_before_warning_entry.grid(row=5, column=1, pady=2, sticky="w")
            tk.Button(input_frame, text="+", command=lambda: self.increment_value(self.rounds_before_warning, 1)).grid(row=5, column=2, padx=2)
            tk.Button(input_frame, text="-", command=lambda: self.decrement_value(self.rounds_before_warning, 1)).grid(row=5, column=3, padx=2)
            self.rounds_before_warning_entry.bind("<FocusOut>", lambda e: self.validate_entry(self.rounds_before_warning, 1, 10))
    ...
    def confirm_submission(self):
        experiment_data = self.get_experiment_data()
        required_fields = ['Subject Name', 'Mode', 'Reinforcement Ratio', 'Total Reinforcements', 'Punishment Period', 'Feed Time', 'Consecutive Warnings Limit', 'Warning Alarm Volume', 'Warning Display Volume', 'Warning Pecks', 'Rounds Before Warning', 'Warning Duration', 'Time Before Warning Signal', 'Warning Signal Position', 'Highlight Warning Signal', 'Is Spot On']
    ...
    def get_experiment_data(self):
        ...
        return {
            ...
            'Rounds Before Warning': self.rounds_before_warning.get(),
    ...
    def load_last_experiment_data(self):
        ...
        if subject:
            ...
            if last_experiment:
                ...
                if 'punishment_period' in last_experiment:
                    self.punishment_period.set(last_experiment['punishment_period'])
                ...
                if 'rounds_before_warning' in last_experiment:
                    self.rounds_before_warning.set(last_experiment['rounds_before_warning'])
    ...
    def submit(self):
        ...
        punishment_period = experiment_data['Punishment Period']
        feed_time = experiment_data['Feed Time']
        is_spot_on = experiment_data['Is Spot On']
        mode_id = next((mode['mode_id'] for mode in self.modes if mode['mode_name'] == experiment_data['Mode']), None)
        ...
        rounds_before_warning = experiment_data['Rounds Before Warning']
        ...
        if subject_name and reinforcement_ratio and total_reinforcements and punishment_period and feed_time and mode_id:
            subject = self.db.select_subject_by_name(subject_name)
            if not subject:
            ...
            experiment_data = {
                ...
                'warning_signal_points': 5,  # Example value
                'warning_pecks': warning_pecks,
                'punishment_period': punishment_period,
                'feed_time': feed_time,
                ...
                'rounds_before_warning': rounds_before_warning,
            ...
            self.db.insert_experiment_with_uuid(experiment_data)
        else:
