...
WHERE 
    r.session_id = '7737ea96-1097-4c38-ae61-1ad1ee8da412'
GROUP BY 
...
SELECT 
    ...
    COUNT(*) AS total_rows, 
    SUM(CASE WHEN warning_terminated THEN 1 ELSE 0 END) AS terminations,
    ROUND(SUM(CASE WHEN warning_terminated THEN 1 ELSE 0 END)::numeric / COUNT(*) * 100, 2) AS termination_percentage
FROM 
    round_results rr
JOIN 
    rounds r ON rr.round_id = r.round_id
